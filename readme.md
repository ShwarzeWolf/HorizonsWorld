...
### Работа из IDE
При желании, можно работать с контейнером с базой данных из IDE. 
Для этого в файле src/scripts.py необходимо заменить второй аргумент функции 
os.getenv на правильную строку соединение.
```
...
Также можно указать следующие параметры: 
   ...
   на стороне Элой, за Карха Солнца
   - **--birthday** TEXT - день рождения героя
...
Помимо этого с помощью команды `python manage.py` можно вызывать скрипт, для создания базы данных и заполнения ее 
тестовыми данными о 6 героях, их девизах, историях, и 10 тестовых сражений.
**Настоятельно не рекоммендуется использваоть при сборке на проде - ВСЕ ДАННЫЕ БУДУТ СТЕРТЫ**
